...
export default function Layout() {
  ...
  return (
    <div className="app-shell flex min-h-screen">
      {/* Sidebar */}
      <aside
        className={`sidebar ${isSidebarCollapsed ? "w-20 px-2" : "w-64 px-6"} py-6 hidden md:flex flex-col transition-all duration-300 relative sticky top-0 h-screen`}
      >
        {/* Toggle Button */}
        <button
        ...
        <div
          className={`mb-10 ${isSidebarCollapsed ? "flex items-center justify-center" : ""}`}
        >
          <h1
            className={`text-2xl font-bold text-gradient ${isSidebarCollapsed ? "text-center text-lg" : ""}`}
          >
        ...
        <nav className="space-y-1.5">
          ...
          {user?.role === "admin" && (
            <>
              <div
                className="pt-4 mt-4"
                style={{ borderTop: "1px solid var(--border-separator)" }}
              >
                {!isSidebarCollapsed && (
